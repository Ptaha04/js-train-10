...
function keysAndValues(obj) {
  if (typeof obj !== "object") {
    ...
    const keysArra = Object.keys(obj);

    const valuesArra = Object.values(obj);

    const array = [keysArra, valuesArra];
    return array;
  }
...
function filterObjectsByKey(arr, key) {
  if (Array.isArray(arr)) {
    const filterFn = (value, index, array) => value.hasOwnProperty(key);
    const filteredKeyArray = arr.filter(filterFn);
